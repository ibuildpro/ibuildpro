# Jerry Jackson 

### I get called when the numbers stop behaving.

Finance data that refuses to match.  
GIS layers with no business meaning.  
Dashboards that look polished but feel suspicious.  
Pipelines that move data faster than people can trust it.

That is my kind of work.

I build the data layer behind serious decisions.  
Clean pipelines.  
Clear logic.  
Trusted reporting.  
No mystery numbers.

## Stack

[![Microsoft Azure](https://custom-icon-badges.demolab.com/badge/Microsoft%20Azure-0089D6?logo=msazure&logoColor=white)](#)
![Azure Data Factory](https://img.shields.io/badge/Azure%20Data%20Factory-0078D4?style=flat-square&logo=microsoftazure&logoColor=white)
![Azure Synapse](https://img.shields.io/badge/Azure%20Synapse-0078D4?style=flat-square&logo=microsoftazure&logoColor=white)
![Databricks](https://img.shields.io/badge/Databricks-FF3621?style=flat-square&logo=databricks&logoColor=white)
![Azure SQL](https://img.shields.io/badge/Azure%20SQL-vCurrent-blue?logo=data:image/svg+xml;base64,)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat-square&logo=postgresql&logoColor=white)
[![Supabase](https://img.shields.io/badge/Supabase-3FCF8E?logo=supabase&logoColor=fff)](#)
[![Power BI](https://custom-icon-badges.demolab.com/badge/Power%20BI-F1C912?logo=power-bi&logoColor=fff)](#)
![GIS](https://img.shields.io/badge/GIS-2F855A?style=flat-square&logo=googleearth&logoColor=white)
[![Python](https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=fff)](#)
[![Pandas](https://img.shields.io/badge/Pandas-150458?logo=pandas&logoColor=fff)](#)
[![Snowflake](https://img.shields.io/badge/Snowflake-29B5E8?logo=snowflake&logoColor=fff)](#)
[![ETL](https://custom-icon-badges.demolab.com/badge/ETL-9370DB?logo=etl-logo&logoColor=fff)](#)
![Data Modeling](https://img.shields.io/badge/Data%20Modeling-1F2937?style=flat-square)
![Data Warehousing](https://img.shields.io/badge/Data%20Warehousing-0F172A?style=flat-square)

---

### If the data is messy, I probably know where it is hiding.
